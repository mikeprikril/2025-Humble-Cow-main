...
public class ArmFlatToHang extends Command {
  ...
  public ArmFlatToHang(ElevatorSubsystem m_elevator, ArmSubsytem m_arm, CommandXboxController m_operatorJoystick) {
  ...
  public void execute() {
    ...
    //arm movement
    if (arm.GetArmEncoderPosition() > Constants.ArmConstants.armFlat + 2*Constants.ElevatorConstants.deadband){
      arm.AutoArmMove(Constants.ArmConstants.ArmUpSpeed*.5);
    }
    else if (arm.GetArmEncoderPosition() < Constants.ArmConstants.armFlat - 2*Constants.ElevatorConstants.deadband){
