...
public class ScoreCoralFast extends Command {
  ...
  public ScoreCoralFast(ElevatorSubsystem m_elevator, ArmSubsytem m_arm, SwerveSubsystem m_swerve, Joystick m_panel) {
  ...
  public void execute() {
    ...
    if (arm.GetArmEncoderPosition() < Constants.ArmConstants.ScorePosition){
      arm.AutoArmMove(Constants.ArmConstants.ArmDownSpeed*.6);
      arm.StopGripper(); 
    }
    else if (arm.GetArmEncoderPosition() > Constants.ArmConstants.ScorePosition && timer.get() < 1.5){
      arm.StopArm();
      arm.GripperSpitOut();
    }
    else {
    ...
    else scoreSpeeds.vxMetersPerSecond = Constants.ArmConstants.DriveBackSpeed*2.5; //move robot
  ...
  public boolean isFinished() {
    return !panel.getRawButton(Constants.OperatorConstants.ScoreAndBackUpFastButton);
  }
}
